...
if (-not $UserObjectId) {
    Write-Host "❌ TEAMS_USER_OBJECT_ID not found in .env file" -ForegroundColor Red
    exit 1
}
if (-not $PolicyName) {
...
Write-Host "`n🚀 Setting up Application Access Policy for Teams transcript access..." -ForegroundColor Green
...
Write-Host "`n📦 Installing/Updating Microsoft Teams PowerShell module..." -ForegroundColor Cyan
try {
...
Write-Host "⚠️  You will be prompted to sign in with a Teams admin account" -ForegroundColor Yellow
try {
...
Write-Host "`n🔧 Creating Application Access Policy..." -ForegroundColor Cyan
try {
...
Write-Host "`n👤 Granting policy to user..." -ForegroundColor Cyan
try {
...
Write-Host "`n🔍 Verifying policy assignment..." -ForegroundColor Cyan
try {
    ...
    if ($userPolicy -and $userPolicy.PolicyName -eq $PolicyName) {
        ...
        Write-Host "⚠️  Policy assignment could not be verified" -ForegroundColor Yellow
    }
} catch {
    Write-Host "ℹ️  Could not verify policy assignment (this is normal and not a problem)" -ForegroundColor Gray
}

# Display completion information
Write-Host "`n🎉 Application Access Policy setup completed!" -ForegroundColor Green
...
Write-Host "`n🧪 Testing Your Setup:" -ForegroundColor Magenta
...
Write-Host "`n🔌 Disconnecting from Microsoft Teams..." -ForegroundColor Cyan
try {
    Disconnect-MicrosoftTeams
    Write-Host "✅ Disconnected successfully" -ForegroundColor Green
} catch {
    Write-Host "⚠️  Disconnect failed (not critical): $($_.Exception.Message)" -ForegroundColor Yellow
}

Write-Host "`n✨ Setup complete! Wait 30 minutes and test with a new meeting." -ForegroundColor Green
